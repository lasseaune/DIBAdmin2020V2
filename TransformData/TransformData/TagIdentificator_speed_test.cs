...
{
    public class IdObjTag1
    {
        public XElement tag { get; set; }
        public List<IdLink> idlink { get; set; }
    }
    public class IdObjTag2
    {
        public XElement tag { get; set; }
        public XElement tagTo { get; set; }
        public XElement tagAlt { get; set; }
        public XElement tagToAlt { get; set; }
        public List<IdLink> idlinks { get; set; }
    }
    public class IdObj
    {
    ...
    { 
        public List<StringRange> stringranges = new List<StringRange>();
        public StringRanges(string value, char split)
        {
            int n =  value.Split(split).Count();
            if (n == 1) {
                StringRange sr = new StringRange { value = value, length = value.Length, partvalue = value };
                stringranges.Add(sr);
            }
            else {
                int pos = 0;
                ...
                {
                    if (pos == 0) {
                        pos = value.IndexOf(split, pos);
                        if (pos == -1) pos = value.Length;
                    }
                    string partvalue = value.Substring(0, pos);
                    StringRange sr = new StringRange { value = value.ToString(), length = partvalue.Length, partvalue = partvalue.ToString() };
                    stringranges.Add(sr);
                }
            }
        }
    ...
    public static class Extentions
    {
        ...
        public static string FormatQNUM(this string s)
        {
            Regex regex = new Regex(@"\d+(([\.\,\-]\d+)+)?((\s+)\%)?");
            s = regex.Replace(s,
                delegate(Match m)
                {
                    string returnValue = Regex.Replace(m.Value, @"\s+|\%"
                        ,delegate(Match d)
                        {
                            switch (d.Value)
                            {
            ...
            s = regex.Replace(s,
                    delegate(Match m)
                    {
                        try
                        {
                            ...
                            return @"QDATE" + returnValue + "DATEQ";
                        }
                        catch
                        {
                            return m.Value;
        ...
        public static string FormatPAHYPU(this string s)
        {
            s = Regex.Replace(s, @"\s+", " ");
        ...
        public static string FormatPAHYPU(this XElement e)
        {
            string s = e.DescendantNodes().OfType<XText>().Select(p => p.ToString()).StringConcatenate();
            return s.FormatPAHYPU();

            
        }
        public static void IdTag2(this IdObjTag2 ido)
        {
            if (ido.tag != null || ido.tagAlt != null )
            {
                XElement tag2 = null;
                if (ido.tagAlt != null) tag2 = ido.tagAlt;
                else tag2 = ido.tag;
                    
                if (ido.tagTo != null || ido.tagToAlt != null)
                {
                    XElement totag2 = null;
                    if (ido.tagToAlt != null) totag2 = ido.tagToAlt;
                    else totag2 = ido.tagTo;

                    List<XElement> tag2s = new List<XElement>();
                    tag2s.Add(tag2);
                    if (tag2.NodesAfterSelf().OfType<XElement>().Where(p => p == totag2).Count() != 0)
                    {
                        tag2s.AddRange(tag2.NodesAfterSelf().OfType<XElement>().TakeWhile(p => p.Name.LocalName == "tag2" && p != totag2));
                    }
                    tag2s.Add(totag2);

                    int x = ido.idlinks.Count();
                }
                else
                {
                    IEnumerable<TagObject> t3 = ido.idlinks.Where(p => (p.tag3 == null ? "" : p.tag3) != "").GroupBy(p => p.tag3).Select(p => p.Key)
                                    .Union(ido.idlinks.Where(p => (p.totag3 == null ? "" : p.totag3) != "").GroupBy(p => p.totag3).Select(p => p.Key)
                                    )
                    ...
                    {
                        var x = (
                            from il in ido.idlinks
                            join t in t3
                            on il.tag3 equals t.value
                            select new { link = il, e = t.tag, type = 0 }
                        ).Union(
                        from il in ido.idlinks
                        join t in t3
                        on il.totag3 equals t.value
                        select new { link = il, e = t.tag, type = 1 }
                        )
                        .GroupBy(p => p.link)
                        .Select(p => new
                        {
                            link = p.Key,
                            tfrom = p.Where(s => s.type == 0 && p.Key.tag3 == s.e.Value).Select(s => s.e).FirstOrDefault(),
                            tto = p.Where(s => s.type == 1 && p.Key.totag3 == s.e.Value).Select(s => s.e).FirstOrDefault()
                        })
                        .ToList();
        ...
        public static XElement SetTag(this string value, IEnumerable<TagObject> tags)
        {
            return tags.Where(p => p.value == value).Select(p => p.tag).FirstOrDefault();
        }
        public static IEnumerable<TagObject> GetTag3(this List<string> values, XElement tag2)
        {
            IEnumerable<StringRange> sr = values.SelectMany(p => new StringRanges(p, '/').stringranges);
            return  
                from t in tag2.Descendants("tag3").Elements("name")
                join s in sr
                on t.Value.Trim().ToLower() equals s.partvalue.Trim().ToLower()
                group t by s.value into q
                select new TagObject {
                    value = q.Key, 
                    tag=q.OrderByDescending(p=>p.Value.Length).Ancestors("tag3").FirstOrDefault()
                };
        }
        public static IEnumerable<TagObject> GetTag2(this List<string> values, XElement tag1)
        {
            return from t in tag1.Descendants("tag2").Elements("name")
                   join s in values
                   on t.Value.Trim().ToLower() equals s
                   select new TagObject { value = s, tag = t.Ancestors("tag2").FirstOrDefault()};
            
        }
        public static void IdTag1(this IdObjTag1 ido)
        {

            IEnumerable<TagObject> t2 = ido.idlink.Where(p => (p.tag2 == null ? "" : p.tag2) != "").GroupBy(p => p.tag2).Select(p => p.Key)
                                        .Union(ido.idlink.Where(p=>(p.tag2_alt==null ? "" : p.tag2_alt)!="").GroupBy(p=> p.tag2_alt).Select(p=>p.Key))
                                        .Union(ido.idlink.Where(p=>(p.totag2==null ? "" : p.totag2)!="").GroupBy(p=> p.totag2).Select(p=>p.Key))
                                        .Union(ido.idlink.Where(p=>(p.totag2_alt==null ? "" : p.totag2_alt)!="").GroupBy(p=> p.totag2_alt).Select(p=>p.Key))
                                        .GroupBy(p=>p)
                                        .Select(p=>p.Key)
                                        .ToList().GetTag2(ido.tag);
                    
                    

            ido.idlink.GroupBy(p=> new
            {
                tag2=p.tag2,
                tag2_alt=p.tag2_alt,
                totag2=p.totag2,
                totag2_alt=p.totag2_alt
            })
            .Select(p=>new IdObjTag2 {
                tag=p.Key.tag2.SetTag(t2),
                tagAlt = p.Key.tag2_alt.SetTag(t2),
                tagTo = p.Key.totag2.SetTag(t2),
                tagToAlt = p.Key.totag2_alt.SetTag(t2),
                idlinks=p.Select(s=>s).ToList() 
            })
            .ToList()
            .ForEach(p=>p.IdTag2());
            ...
            //(
            //    from i in ido.idlink
            //    join t in ido.tag.Descendants("tag2").Elements("name")
            //    on i.tag2.Trim().ToLower() equals t.Value.Trim().ToLower()
            //    where (i.tag2 == null ? "" : i.tag2.Trim()) != ""
            //    select new IdObjTag2
            //    {
            //        idlink = i,
            //        tag = t
            ...
            //    from i in ido.idlink
            //    join t in ido.tag.Descendants("tag2").Elements("name")
            //    on i.tag2_alt.Trim().ToLower() equals t.Value.Trim().ToLower()
            //    where (i.tag2_alt == null ? "" : i.tag2_alt.Trim()) != ""
            //    select new IdObjTag2
            //    {
            //        idlink = i,
            //        tagAlt = t
            ...
            //    from i in ido.idlink
            //    join t in ido.tag.Descendants("tag2").Elements("name")
            //    on i.totag2.Trim().ToLower() equals t.Value.Trim().ToLower()
            //    where (i.totag2 == null ? "" : i.totag2.Trim()) != ""
            //    select new IdObjTag2
            //    {
            //        idlink = i,
            //        tagTo = t
            ...
            //    from i in ido.idlink
            //    join t in ido.tag.Descendants("tag2").Elements("name")
            //    on i.totag2_alt.Trim().ToLower() equals t.Value.Trim().ToLower()
            //    where (i.totag2_alt == null ? "" : i.totag2_alt.Trim()) != ""
            //    select new IdObjTag2
            //    {
            //        idlink = i,
            //        tagToAlt = t
            ...
            //{
            //    idlink = p.Key,
            //    tag = p.Where(s=>s.tag!=null).Select(s=>s.tag.AncestorsAndSelf("tag2").FirstOrDefault()).FirstOrDefault(),
            //    tagAlt = p.Where(s => s.tagAlt != null).Select(s => s.tagAlt.AncestorsAndSelf("tag2").FirstOrDefault()).FirstOrDefault(),
            //    tagTo = p.Where(s => s.tagTo != null).Select(s => s.tagTo.AncestorsAndSelf("tag2").FirstOrDefault()).FirstOrDefault(),
            //    tagToAlt = p.Where(s => s.tagToAlt != null).Select(s => s.tagToAlt.AncestorsAndSelf("tag2").FirstOrDefault()).FirstOrDefault()
            //})
            //.ToList()
            //.ForEach(p => p.IdTag2());
        }
    ...
    public class RegexMatchActions
    {
        ...
        public void Execute(IdentificatorElement element)
        {
            try
            {
                ...
                {
                    //Debug.Print(m.Value);
                    if (m.Value.Trim() == "forvaltningsloven §§ 13 til 13 e") Debug.Print("xxx");
    ...
    public class IdentificatorElements
    {
        ...
        public IdentificatorElements(Identificator identificator, Regex regex, XElement actions)
        {
            SetNumberWords();
            this.identyfiedlinks = new IdentyfiedLinks();

            this.parent = identificator;
            this.rmActions = new RegexMatchActions(regex, actions, this.listOfNumbers);
            this.includes = identificator.Include.Split(';').Where(p => p != "").ToList();
            this.excludes = identificator.Exclude.Split(';').Where(p => p != "").ToList();
            this.breaknodes = identificator.BreakNodes.Split(';').Where(p => p != "").ToList();
            this.nobreaknodes = identificator.NoBreakNodes.Split(';').Where(p => p != "").ToList();
            this.nonmarkupnodes = identificator.NoMarkupNodes.Split(';').Where(p => p != "").ToList();

            (from e in identificator.Document.DescendantsAndSelf()
             join s in this.includes
        ...
        public void Identify()
        {
            ...
            if (parent.IdentifyGlobal)
            {
                ...
                if ((parent.ExternalExtractTags == null ? 0 : parent.ExternalExtractTags.Descendants("tag1").Count()) != 0)
                {
        ...
        {
            //self_tag1
            if (parent.ReferencedTopicsTags == null) return; 
            
            XElement tags = parent.ReferencedTopicsTags;

            var groupedTags = GetIdLinks().GroupBy(p => new
            {
                tag1 = p.tag1,
                tag2 = p.tag2,
                tag2_alt = p.tag2_alt,
            ...
            .Select(p => new
            {
                tag1 = p.Key.tag1,
                tag2 = p.Key.tag2,
                tag2_alt = p.Key.tag2_alt,
                totag2 = p.Key.totag2,
                totag2_alt = p.Key.totag2_alt,
                tag3 = p.Key.tag3,
                totag3 = p.Key.totag3,
                n = p.Select(s=>s).ToList()
            });
            (
                from n in tags.Descendants("tag1").Elements("name")
                join gt in groupedTags
                on n.Value.Trim().ToLower() equals gt.tag1.Trim().ToLower()
                select new {idL = gt, t = n}
            ).ToList()
            .GroupBy(p=>p.t.AncestorsAndSelf("tag1").FirstOrDefault())
            .Select(p=> new IdObjTag1 {
                tag = p.Key,
                idlink = p.SelectMany(s=>s.idL.n.Select(n=>n)).ToList()
            })
            .ToList()
            .ForEach(p => p.IdTag1());
        }
        private void UpdateInternalGlobal()
        {
            try
            {
                ...
                {
                    _InternalTags = new XElement(_InternalTags.Name.LocalName,
                            from n in _InternalTags.Descendants("tag1").Elements("name")
                            join t in GetIdLinks().GroupBy(p => p.tag1).Select(p => p.Key)
                            on n.Value.Trim().ToLower() equals t.Trim().ToLower()
                            select n.Ancestors("tags").GroupBy(p => p).Select(p => p.Key)
                    );

                    if (_InternalTags.Descendants("tag1").Count() == 0) _InternalTags = null;
                }
                ...
                {
                    _ReferencedTopicsTags = new XElement(_ReferencedTopicsTags.Name.LocalName,
                            from n in _ReferencedTopicsTags.Descendants("tag1").Elements("name")
                            join t in GetIdLinks().GroupBy(p => p.tag1).Select(p => p.Key)
                            on n.Value.Trim().ToLower() equals t.Trim().ToLower()
                            select n.Ancestors("tags").GroupBy(p => p).Select(p => p.Key)
                    );

                    if (_ReferencedTopicsTags.Descendants("tag1").Count() == 0) _ReferencedTopicsTags = null;
                }
                if (_ReferencedTopicsTags != null)
                {
    ...
    public class Identificator
    {
        ...
        public XElement ExecuteMark(XElement Document,
                        XElement actions,
                        string Regexp = "",
                        string Language = "no"
            )
        {
            try
            {
                ...
                IdentificatorElements identificatorelements = new IdentificatorElements(this, regex, actions);
                identificatorelements.Mark();

                Document.Descendants()
                    .Where(p => "diblink;dibparameter".Split(';').Contains(p.Name.LocalName))
                    .ToList()
                    .ForEach(p => p.ReplaceWith(p.FormatPAHYPU()));
            ...
            {
                throw new Exception("ExecuteMark - Error: \r\n" + err.Message);
            }
        }
