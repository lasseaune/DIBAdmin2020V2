...
{
    public partial class frmImportLD : Form
    {
                                ...
                                , int level)
        {
            ...
            while (min < max)
            {
                ...
                {
                    if (section.Elements("section").LastOrDefault() == null)
                    {
                        return;
        ...
        private void _v0_GetHeaderType(List<XElement> h)
        {
            ...
            foreach (XElement e in h)
            {
                ...
                if (!found)
                {
                    string testRegexp = @"^(EØS-avtalen|til\skapittel|merknader|rettelser|vedlegg(\s|$)|[^\.]+\s(vedlegg|\(ef\))\s)";
                    Match m = Regex.Match(testValue, testRegexp, RegexOptions.IgnoreCase);
                    if (m.Value != "")
                    {
                    ...
                    while (found)
                    {
                        ...
                        foreach (XElement s in sections.Descendants().Where(p => p.Ancestors().Count() == ansCounter))
                        {
                            ...
                            else
                            {
                                ...
                                XElement ans = s.Ancestors("section").Where(p => !TryConvertToInt(p.Attribute("hid").Value)).FirstOrDefault();
                                if (ans == null) ans = s.NodesBeforeSelf().OfType<XElement>().Where(p => p.Name.LocalName == "section" && !TryConvertToInt(p.Attribute("hid").Value)).FirstOrDefault();
                                    
                                string parentId = ans.Attribute("hid").Value;//s.Parent.Attribute("hid").Value;// == null ? s.Parent.Attribute("tid").Value : s.Parent.Attribute("id").Value;
        ...
        private void GetIndexItems(XElement el, XElement items)
        {
            ...
            int id = 1;
            el.Descendants("section").Where(p=>p.Attribute("id")==null).ToList().ForEach(p=>p.Add(new XAttribute("id", "dib" + (id++).ToString())));
        ...
        private string FindEnd(XDocument topElement)
        {
            ...
            XElement idx = null;
            if (topElement.Descendants().Where(p => TrimElementText(p.ToString()).IndexOf("Databasen sist oppdatert") > 0).Count() > 0)
                idx = topElement.Descendants().Where(p => TrimElementText(p.ToString()).IndexOf("Databasen sist oppdatert") > 0).AncestorsAndSelf("p").First();
       ...
       private string CleanUpInputHtml(string html)
       {
           ...
           try
           {
               ...
               html = Regex.Replace(html, @"\&Delta\;", @"&#x" + ConvertIntToHex(916) + ";");
               html = Regex.Replace(html, @"\&iota\;", @"&#x" + ConvertIntToHex(953) + ";");
               html = Regex.Replace(html, @"\&kappa\;", @"&#x" + ConvertIntToHex(954) + ";");
               html = Regex.Replace(html, @"\&eta\;", @"&#x" + ConvertIntToHex(951) + ";");
               html = Regex.Replace(html, @"\&gamma\;", @"&#x" + ConvertIntToHex(947) + ";");
               html = Regex.Replace(html, @"\&rho\;", @"&#x" + ConvertIntToHex(961) + ";");
               html = Regex.Replace(html, @"\&eth\;", @"&#x" + ConvertIntToHex(240) + ";");
               html = Regex.Replace(html, @"\&sigmaf\;", @"&#x" + ConvertIntToHex(962) + ";");
               html = Regex.Replace(html, @"\&Uuml\;", @"&#x" + ConvertIntToHex(220) + ";");
               html = Regex.Replace(html, @"\&pound\;", @"&#x" + ConvertIntToHex(163) + ";");
               html = Regex.Replace(html, @"\&prime\;", @"&#x" + ConvertIntToHex(8242) + ";");
               html = Regex.Replace(html, @"\&Prime\;", @"&#x" + ConvertIntToHex(8243) + ";");
       ...
       private string ConvertIntToHex(int decValue)
       {
        ...
        private void btnLoadDocument_Click(object sender, EventArgs e)
        {
            ...
            Uri uri = new Uri(adress.Text);
            string err = "";
            XElement html = TransformData.Global.global.GetWebPageHTML(adress.Text, ref err);
            if (html == null)
            {
                MessageBox.Show(err);
                return;
            ...
            html = html.DescendantsAndSelf("html").FirstOrDefault();
            ...
            string tempFolder = _documentLibry + @"temp\";
            //XDocument cleanXML = XDocument.Parse(html.ToString()); 
            XElement cleanXML = html;
            ...
            XElement topElement = cleanXML.XPathSelectElement("body/table[1]/tr[1]/td[2]/table[1]/tr[2]/td[1]");
            ...
            else
            {
                ...
                else if (topElement != null && xd.XPathSelectElement("html/table[1]/tr[1]/td[1]/font[1]/b[1]") == null)
                {
                    ...
                    else if (topElement.Descendants("a").Where(p => (p.Attribute("href") == null ? "" : p.Attribute("href").Value) == "#map0").Count() == 1)
                    {
                        title = topElement.Descendants("a").Where(p => (p.Attribute("href") == null ? "" : p.Attribute("href").Value) == "#map0").First().Value.Trim();

                        if (topElement.Descendants("tr").Where(p => (p.Elements("td").Count() != 0 ? p.Elements("td").ElementAt(0).Value.Trim().ToLower() : "") == "dato:").Count() == 1)
                        {
                            string ldate = topElement.Descendants("tr").Where(p => (p.Elements("td").Count() != 0 ? p.Elements("td").ElementAt(0).Value.Trim().ToLower() : "") == "dato:").First().Elements("td").ElementAt(1).Value.Trim();
                            title = ldate + ":" + title;
                        }
                    }
                    else if (cleanXML.XPathSelectElement("body/table[1]/tr[1]/td[2]/table[1]/tr[2]/td[2]") != null)
                    {
                        topElement = cleanXML.XPathSelectElement("body/table[1]/tr[1]/td[2]/table[1]/tr[2]/td[2]");
                        title = topElement.Value;
                    }
                    else if (topElement.Elements("h3").Count() != 0)
                    {
                        title = topElement.Elements("h3").First().Value;
                    }
        ...
        {
            List<XElement> pbs = part.Descendants("pb").ToList();
            while (pbs.Count() != 0)
            {
                part.Descendants("pb").First().ReplaceWith(part.Descendants("pb").First().Nodes());
                pbs = part.Descendants("pb").ToList();
            }

            List<XElement> fns = part.Descendants("fn").ToList();
            while (fns.Count() != 0)
            {
                part.Descendants("fn").First().ReplaceWith(part.Descendants("fn").First().Nodes());
                fns = part.Descendants("fn").ToList();
            }

            part.Descendants("a").Where(p=>(p.DescendantNodes().OfType<XText>().Count() == 0 ? "" : p.DescendantNodes().OfType<XText>().Select(s=>s.ToString()).StringConcatenate().Trim())=="").Remove();
            part.Descendants("strong").Where(p => !p.HasElements && p.Value.Trim() == "").Remove();
            ...
            part.Elements().Where(p => p.Name == "p" &&  p.DescendantNodes().OfType<XText>().Select(q=>q.ToString()).StringConcatenate().Trim() == "").Remove();
            ...
            {
                if (e.Name.ToString() == "p" && (e.Elements("strong").Count() != 0 || e.Elements("b").Count() != 0) && (e.Elements("i").Count() != 0 || e.Elements("em").Count() != 0))
                {
                ...
                else if (e.Name.ToString() == "p" && (e.Elements("strong").Count() != 0 || e.Elements("b").Count() != 0 ) &&  (e.Elements("i").Count() == 0 || e.Elements("em").Count() == 0)) 
                {
                    newPart.Add(new XElement("title",
                            new XElement("t", e.Elements().Where(p=>
                                p.Name.LocalName == "strong"
                                || p.Name.LocalName == "b"
                                ).First().Value)));
                    XElement newP = new XElement(e);
                    newP.Elements("strong").Remove();
        ...
        private static XElement EvalNode(XElement el)
        {
            ...
            foreach (XNode xe in el.Nodes())
            {
                if (xe.NodeType == XmlNodeType.Text)
                {
                    ...
                    returnElement.Add(new XElement("t", xe));
                }
                else if (xe.NodeType == XmlNodeType.Element)
                {
                    ...
                    else if (myEl.Name.LocalName == "sub")
                    {
                        ...
                        && (
                            myEl.Name.LocalName == "em"))
                    {
                        returnElement.Add(new XElement("t", myEl.Nodes(),
        ...
        private void btnTOC_Click(object sender, EventArgs e)
        {
            ...
            if (textAdress.SelectedIndex != -1)
            {
                ...
                if (idx2 != null)
                {
                    ...
                    if (toc != null)
                    {
                        List<SectionInfo> si = new List<SectionInfo>();
                        ...
                        foreach (XElement section in toc.DescendantNodesAndSelf())
                        {
                            ...
                            int idx = 0;
                            SectionInfo sItem = si.Find(
                               delegate(SectionInfo p)
                               {
                                   return p.id == href;
                               });
                            if (sItem == null)
                            {
                                sItem = new SectionInfo();
                                sItem.id = href;
                                idx = 0;
                                si.Add(sItem);
                            }
                            ...
                            while (ant == 0)
                            {
                                ant = xd.Descendants("a").Where(p => 
                                    (
                                        (p.Attribute("name") == null ? "" : p.Attribute("name").Value.Trim()) == href
                                        ||  (p.Attribute("name") == null ? "" : p.Attribute("name").Value.Trim().Replace(" ","")) == org_href.Replace(" ","")
                                    )
                                    && Convert.ToInt32(p.Attribute("idx").Value) > idx)
                                    .Count();

                                if (ant != 0)
                                {
                                    string from = "";
                                    XElement a = xd.Descendants("a").Where(p =>
                                        (
                                            (p.Attribute("name") == null ? "" : p.Attribute("name").Value.Trim()) == href
                                            || (p.Attribute("name") == null ? "" : p.Attribute("name").Value.Trim()) == org_href
                                        )
                                        && Convert.ToInt32(p.Attribute("idx").Value) > idx)
                                        .First();
                                    switch (a.Parent.Name.ToString())
                                    {
                                        ...
                                        default:
                                            if (a.Ancestors().Where(p =>
                                                    ...
                                                    || p.Name == "h5").Count() != 0)
                                            {
                                                from = a.Ancestors().Where(p =>
                                                    p.Name == "p"
                                                    || p.Name == "h1"
                                                    || p.Name == "h2"
                                                    || p.Name == "h3"
                                                    || p.Name == "h4"
                                                    || p.Name == "h5").First().Attribute("idx").Value;
                                            }
                                            else
                                            {
                                    ...
                                    sItem.idx.Add(Convert.ToInt32(a.Attribute("idx").Value));
