...
namespace TransformData
{
    ...
    public partial class frmHtmlImport0 : Form
    {
        ...
        public frmHtmlImport0(ref ToolStripProgressBar p, ref ToolStripStatusLabel l)
        {
            ...
            _htmlText.TextChanged +=new EventHandler(_htmlText_TextChanged);
            btnHtmlEdit.Enabled = false;
        ...
        private void LoadExisting()
        {
            ...
            XElement docs = XElement.Load(@"C:\DIB-Import\forarbeidertoimport.xml");
        ...
        public void OpenDocument(global.existing tag)
        {
            XElement myRes = null;
            XDocument xmlDoc = null;
            if (File.Exists(global._dbPath + "import.xml"))
        ...
        {
            string regexpXML = @"C:\Documents and Settings\Lasse Aune\Mine dokumenter\Visual Studio 2010\Projects\TransformData\TransformData\xml\regexpEx01.xml";
        ...
        private void _htmlText_TextChanged(object sender, EventArgs e)
        {
            if ((_htmlText.Text == null ? "" : _htmlText.Text) != "")
            {
                btnHtmlEdit.Enabled = true;
        ...
        private void CreateWebBrowser()
        {
            ...
            myWebBrowser.Navigated += new WebBrowserNavigatedEventHandler(myWebBrowser_Navigated);
        ...
        {
            if (_DocumentCompletedAction == WebBrowserActions.import)
            {
                System.Windows.Forms.HtmlDocument h = myWebBrowser.Document;
                HtmlElementCollection he = h.GetElementsByTagName("html");
                foreach (HtmlElement hel in he)
                {
                    _DocumentCompletedAction = WebBrowserActions.normalnavigation;
                    string path = @"c:\_temp\xx\";
                    string fileName = HAP(hel.OuterHtml, e.Url.ToString(), path);
                    adress.Text = path + fileName;
                    myWebBrowser.Navigate(path + fileName);
                }
            }
            else if (_DocumentCompletedAction == WebBrowserActions.loadedit || _DocumentCompletedAction == WebBrowserActions.loadedithidden)
            {
        ...
        private void StopMark()
        {
            ...
            //if (_mElement != null)
            //{
            //    if (this.elementStyles.ContainsKey(_mElement))
            //    {
            //        string style = this.elementStyles[_mElement];
            //        this.elementStyles.Remove(_mElement);
            //        _mElement.Style = style;
        ...
        private HtmlNode GetDocumentContent(HtmlAgilityPack.HtmlDocument h)
        {
            ...
            {
                if (h.DocumentNode.Descendants().Where(p => (p.Attributes["class"] != null ? p.Attributes["class"].Value : "") == tbDocumentConent.Text.ToString()).Count() != 1)
                {
                ...
                else
                {
                    ...
                    //returnValue = returnValue.ChildNodes.Descendants("div").Where(p => p.Attributes["class"] != null).First();
                }
            }
            else if (tbDocumentConent.Text.ToString().Split('|').Count() == 2)
            {
                ...
                //foreach (HtmlNode d in h.DocumentNode.Descendants().Where(p => p.NodeType == HtmlNodeType.Comment && p.InnerText.Contains(" INNHOLD ")))
                //{
                //    string x = d.InnerHtml;
                //}
                            ...
                            if (!att.Value.Trim().StartsWith("#"))
                            {
                                ...
                                if (filename != "")
                                {
                                    ...
                                    if (!File.Exists(savePath + filename))
                                    {
                                        ...
                                        tr.Close();
                                        global.metods met = new global.metods();
        ...
        private List<global.cssValues> _CSS = null;
        ...
        private void btnImport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                _CSS = new List<global.cssValues>();
                ...
                {
                    if (hrefs.Descendants("part").Count() == 0)
                    {
                        return;
                    }
                    else
                    {
                        importAdress = hrefs.Descendants("part").First().Element("href").Value;
                    }
                }
                else
                {
                ...
                if (Directory.Exists(tbTargetDir.Text)) Directory.Delete(tbTargetDir.Text, true);
                _PROGRESS.Value = 1;
                
                btnImportIndex_Click(sender, e);
                
                _PROGRESS.Value = 2;
                if (_myXDoc.GetXDoc().Elements().Where(p=>p.Name.LocalName=="parts").Count() == 0) return;
                ...
                if (_STYLES.Count() != 0)
                {
                    TextWriter tw = new StreamWriter(path + @"style.css");
                ...
                foreach (XElement href in hrefs.Descendants("part"))
                {
                    ...
                    XElement fd = XElement.Load(tbTargetDir.Text +  href.Element("xml").Value);
                ...
                {
                    XElement last = XElement.Load(tbTargetDir.Text + number.Attribute("xml").Value);
                    ...
                    last.Save(tbTargetDir.Text + number.Attribute("xml").Value);
                }
            ...
            , XElement part)
        {
            ...
            XElement El = XElement.Parse(hnHtml);
        ...
        private XElement CheckNodes(XElement el)
        {
            ...
            for (int i = n; i > 0; i--)
            {
                ...
                if (e.Elements("div").Where(p => (p.Attribute("class") == null ? "" : p.Attribute("class").Value) == "K-FIG").Count()!=0)
                {
                    ...
                    string areaFolder = dInfo.Element("foldername").Value;
        ...
        private void btnHtmlEdit_Click(object sender, EventArgs e)
        {
            
            if ((_htmlText.Text == null ? "" : _htmlText.Text) != "")
            {
                _sourceAdress = myWebBrowser.Document.Url.AbsoluteUri.ToString();

                myWebBrowser.Navigate(System.Windows.Forms.Application.StartupPath + @"\html\full.html");
                myWebBrowser.ObjectForScripting = this;
                _DocumentCompletedAction = WebBrowserActions.loadedithidden;
            }
        }
        ...
        private void btnSaveHtml_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (_myXDoc.GetXDoc() != null)
                {
                    XElement x = _myXDoc.GetXDoc();
                    XElement parts = null;
                    XElement part = null;
                    XElement href = null;
                    if (x.Elements("parts").Count() == 0)
                    {
                        parts = new XElement("parts");
                        parts.Add(x);
                    }
                    else
                        parts = x.Elements("parts").First();

                    if (parts.Elements("part").Count() == 0)
                    {
                        part = new XElement("part");
                        parts.Add(part);
                    }
                    else
                        part = parts.Elements("part").First();

                    if (part.Elements("name").Count() == 0)
                    {
                        part.Add(new XElement(x.Elements("name").First()));
                    }

                    if (part.Elements("href").Count() == 0)
                    {
                        href = new XElement("href", _sourceAdress);
                        part.Add(href);
                    }
                    else
                    {
                        href = part.Elements("href").First();
                        href.Value = _sourceAdress;
                    }

                    string url = part.Element("href").Value;
                    Uri uri = new Uri(url);
                    string path = tbTargetDir.Text;
                    if (!path.EndsWith(@"\"))
                    {
                    ...
                    HtmlAgilityPack.HtmlDocument hx = new HtmlAgilityPack.HtmlDocument();
                    hx.LoadHtml(_htmlText.Text);
                    HtmlNode hn = hx.DocumentNode;
                    ...
                    if (!uri.LocalPath.EndsWith("/"))
                    {
                        fname = Path.GetFileName(uri.LocalPath).Replace(Path.GetExtension(uri.LocalPath), "");
                    }

                    string hnHtml = hn.OuterHtml;

                    XElement El = SaveCleanHtml(hnHtml
                        , 1
                        , path
                        , uri
                        , fname
                    ...
                    XElement lroot = new XElement("root");
                    int elementCounter = 0;
                    SaveXMLResult(El, path, fname, part, ref root, lroot, true, ref elementCounter, 1);
        ...
        private void btnOpenDoc_Click(object sender, EventArgs e)
        {
            ...
            frmGetDocuments f = new frmGetDocuments(this, regEx);
            f.ShowDialog();
        ...
        private void OpenDocumentInBrowser(XElement myRes)
        {
            ...
            if (myRes.Descendants("part").Where(p => p.Attribute("no") == null).Count() != 0)
            {
                foreach (XElement part in myRes.Descendants("part"))
                {
            ...
            foreach (XElement part in myRes.Descendants("part"))
            {
                global.ComboboxItem item = new global.ComboboxItem();
                item.Text = part.Element("name").Value;
                item.Value = Convert.ToInt32(part.Attribute("no").Value);
                cbLinks.Items.Add(item);
                //cbLinks.Items.Add(part.Element("name") == null ? part.Element("title").Value :  part.Element("name").Value);
            }
        ...
        private string HAP(string html, string url, string path)
        {
            string returnValue = "";
            try
            {
                        ...
                        case "link":
                            if (n.Attributes["href"] != null)
                            {
                                ...
                                if (!att.Value.Trim().StartsWith("#"))
                                {
                                    Uri r_uri = new Uri(uri, att.Value);
                                    string filename = Path.GetFileName(r_uri.LocalPath);
                                    att.Value = subPath + filename;
                                    mDownloadFile(r_uri.AbsoluteUri, subPath, filename);
                                }
                            }
                            break;
                        case "img":
                ...
                HtmlNode hn = h.DocumentNode.Descendants("div").Where(p => (p.Attributes["class"] != null ? p.Attributes["class"].Value : "") == "R-SUMCHILD").First();
                XElement e = XElement.Parse(hn.OuterHtml);
                ///e.Save(path + "xxx.xml");
                h.Save(path + Path.GetFileName(uri.LocalPath));
                returnValue = Path.GetFileName(uri.LocalPath);
            }
        ...
        private XElement ReadDocument()
        {
            ...
            else if (rbArea2.Checked)
            {
                ...
                {
                    MessageBox.Show("Kan ikke importere denne siden!");
                    return null;
                }
        ...
        private void btnImportIndex_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (tbSelectName.Text == "") return;
                HtmlNode hn = null;
                if (rbId.Checked)
                {
                    if (h.DocumentNode.Descendants().Where(p =>(p.Attributes["id"] != null ? p.Attributes["id"].Value : "").Trim().ToLower() == "innholdsfortegnelse").Count() == 0)
                    {
                        foreach (HtmlNode xx in h.DocumentNode.Descendants())
                        {
                            if ((xx.Attributes["id"] != null ? xx.Attributes["id"].Value : "") != "")
                            {
                                if ((xx.Attributes["id"] != null ? xx.Attributes["id"].Value : "") == "innholdsfortegnelse")
                                {
                                    Debug.Print((xx.Attributes["id"] != null ? xx.Attributes["id"].Value : ""));
                                    hn = xx;
                                }
                            }
                        }
                    ...
                    if (hn == null)
                    {
                        if (h.DocumentNode.Descendants().Where(p => (p.Attributes["id"] != null ? p.Attributes["id"].Value : "").Trim().ToLower() == tbSelectName.Text.Trim().ToLower()).Count() == 0)
                        {
                            MessageBox.Show("Kan ikke finne ett element!");
                            return;
                        }
                        ...
                        {
                            //hn = h.DocumentNode.Descendants("li").Where(p => p.InnerText.Trim().ToLower() == "innhaldsliste" || p.InnerText.Trim().ToLower() == "innholdsfortegnelse").First().ParentNode;
                            hn = h.DocumentNode.Descendants().Where(p => (p.Attributes["id"] != null ? p.Attributes["id"].Value : "").Trim().ToLower() == tbSelectName.Text.Trim().ToLower()).First();
                        }
                    }
                }
                else if (rbClass.Checked)
                {
                    if (h.DocumentNode.Descendants().Where(p => (p.Attributes["class"] != null ? p.Attributes["class"].Value : "") == tbSelectName.Text).Count() != 1)
                    {
                        MessageBox.Show("Kan ikke finne ett element!");
                        return;
                    }
                    ...
                    {
                    hn = h.DocumentNode.Descendants().Where(p => (p.Attributes["class"] != null ? p.Attributes["class"].Value : "") == tbSelectName.Text).First();
                    }
                }
                cbLinks.Items.Clear();
                XElement ls = new XElement("parts");

                if (adress.Text.IndexOf("www.regjeringen.no") != -1)
                {
                    HtmlNode ul = null;
                    if (hn.Descendants("ul").Count() == 0)
                    {
                        return;
                    }
                    ...
                    foreach (HtmlNode n in ul.Descendants("li").Where(p => p.Ancestors("ul").Count() == 1))
                    {
                        ...
                        else if (n.Descendants("a").Count() != 0)
                        {
                            ...
                            string partText = n.InnerText.Trim();
                            partText = Regex.Replace(partText, @"(\r\n)+", " ");
                            partText = Regex.Replace(partText, @"\t+", " ");
                            partText = Regex.Replace(partText, @"\s+", " ");

                            if (partText.EndsWith(".."))
                            {
                                if (n.Attributes["title"] != null)
                                {
                                    partText = n.Attributes["title"].Value.Trim();
                                    partText = Regex.Replace(partText, @"(\r\n)+", " ");
                                    partText = Regex.Replace(partText, @"\t+", " ");
                                    partText = Regex.Replace(partText, @"\s+", " ");
                                }
                ...
                _PROGRESS.Maximum = ls.Elements().Count() + 1;
                _PROGRESS.Value = 1;

                XElement x = _myXDoc.GetXDoc();
                if (x.Elements(ls.Name).Count() == 0)
                {
                    x.Add(ls);
                }
                else
                {
                    x.Elements(ls.Name).Remove();
                    x.Add(ls);
                }

                cbLinks.Tag = ls;
                XElement El = XElement.Parse(hn.OuterHtml);
                El.Save(path + Path.GetFileName(uri.LocalPath));
        ...
        private void btnTargetDir_Click(object sender, EventArgs e)
        {
            ...
            {
                tbTargetDir.Text = folderBrowserDialog1.SelectedPath;
            }
        }

        private void cbLinks_SelectedIndexChanged(object sender, EventArgs e)
        {
            XElement El = _myXDoc.GetXDoc();
            if (El == null) return;
            if (cbLinks.SelectedItem != null)
            {
                global.ComboboxItem item = cbLinks.SelectedItem as global.ComboboxItem;
        ...
        private void btnSectionDocument_Click(object sender, EventArgs e)
        {
            ...
            if (cbLinks.SelectedItem == null) { MessageBox.Show("Velg fil!"); return; }
        ...
        private void IdentyfiAllSections(XElement xd)
        {
            foreach (XElement s in xd.DescendantsAndSelf("section"))
            {
                string lastId = "";
                string newId = "";
                if (!(s.Attribute("id") == null ? "" : s.Attribute("id").Value).StartsWith("KAP"))
                {
                    if (s.Parent.Attribute("id") != null)
                    {
                        ...
                        while (p != null)
                        {
                            if (p.NodeType == XmlNodeType.Element)
                            {
                                ...
                                if (test.Name.LocalName == "section")
                                {
                                    if ((test.Attribute("id") == null ? "" : test.Attribute("id").Value).StartsWith("KAP"))
                                    {
                                ...
                                {
                                    if ((test.Parent.Attribute("id") == null ? "" : test.Parent.Attribute("id").Value).StartsWith("KAP"))
                                    {
                        ...
                        //Debug.Print(s.Parent.Attribute("id").Value);
                    }
                }
            }
        ...
        private void rbShowWeb_CheckedChanged(object sender, EventArgs e)
        {
            if (rbShowWeb.Checked)
                cbLinks_SelectedIndexChanged(sender, e);
        }
        ...
        private void btnSection_Click(object sender, EventArgs e)
        {
            ...
            global.ComboboxItem item = cbLinks.SelectedItem as global.ComboboxItem;
            XElement El = _myXDoc.GetXDoc();
            XElement link = El.Descendants("part").Where(p => p.Attribute("no").Value == item.Value.ToString()).First();
            string fileName = "";
            if (rbShowFinal.Checked)
            {
                if (link.Element("final") == null)
                {
                    if (link.Element("xml") == null)
                    {
                        MessageBox.Show("Importer dokument!");
                        return;
                    }
                    else
                        fileName = link.Element("xml").Value;
                }
                else
                    fileName = link.Element("final").Value;
            }
            else
            {
                if (link.Element("xml") == null)
            ...
            if (fileName != "")
            {
                if (!File.Exists(tbTargetDir.Text + fileName)) { MessageBox.Show("XML-fil finnes ikke, importer dokument!"); return; }
                FileInfo fi = new FileInfo(tbTargetDir.Text + fileName);
                if (myWebBrowser == null) CreateWebBrowser();
                adress.Text = tbTargetDir.Text + fileName;
                XElement xd = XElement.Load(adress.Text);
                HtmlHierarchy h = new HtmlHierarchy();
                int counter = 0;
                h.SetElementId(xd, ref counter);

                AnalyseXML ax = new AnalyseXML(xd); ;
                XElement result = new XElement("x");
                while (result != null)
                {
                    result = null;
                    ax = new AnalyseXML(xd);
                    int idx = 0;
                    result = ax.AnalyseResult(ref idx);
                    
                    if (result != null)
                    {
                        frmAnalyseXML f = new frmAnalyseXML(result);
                        f.ShowDialog();
                        ...
                        if (f.XML.Name.LocalName == "document")
                            xd = f.XML;
                        else
                        {
                            if (xd.Descendants()
                                .Where(p => (p.Attribute("idx") == null ? "" : p.Attribute("idx").Value) == f.XML.Attribute("idx").Value)
                                .Count() == 1)
                            {
                                XElement newEl = xd.Descendants()
                                .Where(p => (p.Attribute("idx") == null ? "" : p.Attribute("idx").Value) == f.XML.Attribute("idx").Value).First();
                                newEl.ReplaceAll(new XElement(f.XML));
                            }
                        }
                    }
                }
                xd.DescendantsAndSelf().Attributes("inspected").Remove();
                string path = global._dbPath + El.Element("foldername").Value + @"\final\";
                ...
                xd.Save(path + fi.Name);
                if (link.Element("final") == null)
                    link.Add(new XElement("final", @"final\" + fi.Name ));
                else
                    link.Element("final").Value = @"final\" + fi.Name;
        ...
        private void btnNew_Click(object sender, EventArgs e)
        {
            ...
            if (title != "")
            {
                ...
                if (global.m_TOPICS.Where(t => t.name1 + "_" + t.number + "_" + t.type + "_" + t.year1 + "_" + t.year2 == id.ToLower()).Count() == 1)
                {
                    topic_id = global.m_TOPICS.Where(t => t.name1 + "_" + t.number + "_" + t.type + "_" + t.year1 + "_" + t.year2 == id.ToLower()).ElementAt(0).topic_id;
                }
            }
        ...
        private void btnEditDocument_Click(object sender, EventArgs e)
        {
            ...
            if (fileName != "")
            {
                ...
                frmAnalyseXML f = new frmAnalyseXML(xd);
                f.ShowDialog();
        ...
        private void rbShowTransform_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                string fileName = "";
                XElement l = El.Descendants("part").Where(p => p.Element("name").Value == cbLinks.SelectedItem.ToString().Trim()).First();
                ...
                string path = @"C:\Documents and Settings\Lasse Aune\Mine dokumenter\Visual Studio 2010\Projects\TransformData\TransformData";
        ...
        private XElement GetContentDocument(XElement document)
        {
        ...
        private XElement GetContent(XElement documents)
        {

            XElement content = new XElement("content");
            foreach (XElement e in documents.Elements("document").Elements("section"))
            {
                e.SetSectionID();
                XElement item = new XElement("item",
                                    new XAttribute("id", e.Attribute("id").Value),
        ...
        private void btnCreateTot_Click(object sender, EventArgs e)
        {
            ...
            foreach (XElement part in el.Descendants().Where(p => p.Name.LocalName == "part"))
            {
                ...
                //if (document.Name.LocalName == "document" && (document.Elements().Count() == 0 ? "" : document.Elements().First().Name.LocalName) != "title")
                //{
                //    document.ReplaceWith(new XElement("docuement", document.Attributes(), document.Nodes()));
                //}

                //Id oppdateres for alle seksjoner

                
            }
            
            foreach (XElement part in el.Descendants().Where(p=>p.Name.LocalName == "part" ||p.Name.LocalName == "container"))
            {
                if (part.Name == "container")
                {
                    item = new XElement("item"
                        , new XAttribute("text", part.Element("name").Value)
                        );
                    content.Add(item);
                }
                else if (part.Name == "part")
                {
                    ...
                    XElement c = GetContentDocument(document);
                    if (item != null)
                        item.Add(c.Nodes());
                    else
                        content.Add(c.Nodes());
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (fileName != "")
            {
                ...
                XElement section = xd.DescendantsAndSelf("section").Where(p => p.Elements("section").Count() == 0).First();
                XElement html = new XElement("html");
                html.Add(section.Nodes().Where(p => (p.NodeType == XmlNodeType.Element ? ((XElement)p).Name.LocalName : "") != "title"));
                _EDITITEM = new EditItem();
                _EDITITEM.partxml = new XElement(section);
                _EDITITEM.partxml.Nodes().Where(p => (p.NodeType == XmlNodeType.Element ? ((XElement)p).Name.LocalName : "") != "title").Remove();
                _EDITITEM.partname = fileName;
                _EDITITEM.sectionId = section.Attribute("id").Value;

                _htmlText.Text = html.ToString();
        ...
        private void btnEditPart_Click(object sender, EventArgs e)
        {
            ...
            string filename = link.EditFinalOrXML();
            ...
            string targetFolder = tbTargetDir.Text + @"output\";
            frmSectionEdit f = new frmSectionEdit(filename, targetFolder, topic);
            f.MdiParent = this.MdiParent;
            f.Show();

        }

        private void btnHtmlSave_Click(object sender, EventArgs e)
        {
            System.Windows.Forms.HtmlDocument doc = myWebBrowser.Document;
            _htmlText.Text = doc.InvokeScript("save").ToString();
            ...
            if (_EDITITEM != null)
            {
                if (MessageBox.Show("Vil du overskrive final!", "Lagre fil", MessageBoxButtons.OKCancel) == DialogResult.OK)
                {
                    XElement htmlPart = XElement.Parse("<html>" + _htmlText.Text + "</html>");
                    _EDITITEM.partxml.Add(htmlPart.Nodes());

                    XElement result = BuildSectionsFromHX(_EDITITEM.partxml, _EDITITEM.sectionId.Replace("KAP", ""));

                    FileInfo fi = new FileInfo(tbTargetDir.Text + _EDITITEM.partname.ToString());
                    XElement part = XElement.Load(tbTargetDir.Text + _EDITITEM.partname.ToString());
                    if (part.DescendantsAndSelf("section").Where(p => (p.Attribute("id") == null ? "" : p.Attribute("id").Value) == _EDITITEM.sectionId).Count() == 1)
                    {
                        part.DescendantsAndSelf("section").Where(p => (p.Attribute("id") == null ? "" : p.Attribute("id").Value) == _EDITITEM.sectionId).First().ReplaceWith(new XElement(result));
                    }

                    XElement xmlPart = _myXDoc.GetXDoc().Descendants("part").Where(p => p.Element("xml").Value == _EDITITEM.partname).First();
                    if (!xmlPart.PartElementSave("final", part, fi.Name))
                    {
                        MessageBox.Show("Filen ble ikke lagret!");
                    }
                    else
                    {
                        _myXDoc.SetXDoc(xmlPart.Ancestors("document").First());
