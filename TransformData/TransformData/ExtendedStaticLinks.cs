...
namespace DIB.RegExp.ExternalStaticLinks
{
    ...
    public class ExtendedStaticLinks
    {
        ...
        public ExtendedStaticLinks(int server)
        {
            _SERVER = server;
        }
        ...
        public List<ExternalStaticLinksEx1.iObject> _lObjects = new List<ExternalStaticLinksEx1.iObject>();

        public List<ExternalStaticLinksEx1.rQuery> _list_rQuery = new List<ExternalStaticLinksEx1.rQuery>();




        private List<ExternalStaticLinksEx1.iObjectEx> _iObjectEx = null;
        private XElement _iObjectXML = null;

        private string BuildTag(ExternalStaticLinksEx1.actionObject ao, string tags, string trim)
        {
            string returnValue = "";
        ...
        private void ExecuteActions(XElement action, ref ExternalStaticLinksEx1.actionObject ao, ref List<xRange> container)
        {
            try
            {
        ...
        private void ActionIf(XElement a, ref ExternalStaticLinksEx1.actionObject ao, ref List<xRange> container)
        {
            try
            {
        ...
        private bool ActionMatch(XElement a, ref ExternalStaticLinksEx1.actionObject ao, ref List<xRange> container)
        {
        ...
        private string GetLovDate(ref ExternalStaticLinksEx1.actionObject ao, string values)
        {
        ...
        private bool SetTag(ref ExternalStaticLinksEx1.actionObject ao, string tag, string value)
        {
            bool returnValue = false;
        ...
        private void ActionGet(XElement a, ref ExternalStaticLinksEx1.actionObject ao, ref List<xRange> container)
        {
            try
            {
        ...
        private void ActionMark(XElement a, ref ExternalStaticLinksEx1.actionObject ao, ref List<xRange> container)
        {
            try
            {
                if ((ao.li != null ? ao.li.Count() : 0) != 0)
                {
                    ...
                    if (container.Count() != 0)
                    {
                        ...
                        if (range != null)
                        {
                            ...
                            for (int i = 0; i < n; i++)
                            {
                                ExternalStaticLinksEx1.linkInfo li = ao.li[i];
        ...
        private void ActionForeach(XElement a, ref ExternalStaticLinksEx1.actionObject ao, ref List<xRange> container)
        {
            try
            {
                if (a.Attribute("querytype").Value == "group")
                {
                    ...
                    for (int i = 0; i < n; i++)
                    {
                        ...
                        {
                            ExternalStaticLinksEx1.actionObject su = new ExternalStaticLinksEx1.actionObject(ao, c, a.Attribute("groups").Value);
                            su.offset = ao.offset;
                            su.start = ao.start;
                            su.m = ao.m;
                ...
                else if (a.Attribute("querytype").Value == "subquery")
                {
                    ...
                    {
                        Match m = mc[i];
                        ExternalStaticLinksEx1.actionObject su = new ExternalStaticLinksEx1.actionObject(ao, m);
        ...
        public bool SetRegexpList(Dictionary<string, string> regexps)
        {
            ...
            try
            {
                List<ExternalStaticLinksEx1.rQuery> rl = _data.LoadTokenRegExp(regexps);
                _list_rQuery = rl;
                returnValue = true;
            }
            catch (SystemException e)
        ...
        {
            _iObjectEx = new List<ExternalStaticLinksEx1.iObjectEx>();
        ...
        private bool ActionIdentify(XElement a, ref ExternalStaticLinksEx1.actionObject ao, ref List<xRange> container, string tag, string value)
        {
        ...
        private bool IdentifyTag1(ref ExternalStaticLinksEx1.actionObject ao)
        {
            bool returnValue = false;
            try
            {
                string tag1 = ao.tag1;
                List<ExternalStaticLinksEx1.linkInfo> _newli = new List<ExternalStaticLinksEx1.linkInfo>();
                bool found = false;
                int n = _lObjects.Where(p => p.tag == tag1 && p.lang == _lang).Count();
                for (int i = 0; i < n; i++)
                //foreach (iObject io in _lObjects.Where(p => p.tag == tag1 && p.lang == _lang ))
                {
                    ExternalStaticLinksEx1.iObject io = _lObjects.Where(p => p.tag == tag1 && p.lang == _lang).ElementAt(i);
                    if (io.id != "")
                    {
                        found = true;
                        ExternalStaticLinksEx1.linkInfo li = new ExternalStaticLinksEx1.linkInfo();
                        li.topic_id = io.id;
                ...
                if (!found)
                {
                    ...
                    List<ExternalStaticLinksEx1.iObject> ioNew = _data.GetParentEx(ao, tag1, _lang, _SERVER);
                    n = ioNew.Count();
                    if (n != 0)
                    {
                        ...
                        for (int i = 0; i < n; i++)
                        {
                            ExternalStaticLinksEx1.iObject io = ioNew.ElementAt(i);
                            if (io.id != "")
                            {
                                found = true;
                                ExternalStaticLinksEx1.linkInfo li = new ExternalStaticLinksEx1.linkInfo();
                ...
                if (_newli.Count() != 0)
                {
                    ao.li = new List<ExternalStaticLinksEx1.linkInfo>(_newli);
                    returnValue = true;
                }
        ...
        private bool IdentifyTag2(ref ExternalStaticLinksEx1.actionObject ao)
        {
            bool returnValue = false;
            try
            {
                string tag1 = ao.tag1;
                string tag2 = ao.tag2;
                List<ExternalStaticLinksEx1.linkInfo> _li = ao.li;
                List<ExternalStaticLinksEx1.linkInfo> _newli = new List<ExternalStaticLinksEx1.linkInfo>();
                int n = _lObjects.Where(p => p.tag == tag1 && p.lang == _lang && p.id != "").Count();
                for (int i = 0; i < n; i++)
                {
                    ExternalStaticLinksEx1.iObject io = _lObjects.Where(p => p.tag == tag1 && p.lang == _lang && p.id != "").ElementAt(i);
                    bool found = false;
                    int nn = io.n.Where(p => p.tag == tag2).Count();
                    for (int ii = 0; ii < nn; ii++)
                    {
                        ExternalStaticLinksEx1.iObject ioBm = io.n.Where(p => p.tag == tag2).ElementAt(ii);
                        found = true;
                        if (ioBm.id != "")
                        {
                            ExternalStaticLinksEx1.linkInfo li = new ExternalStaticLinksEx1.linkInfo();
                    ...
                    if (!found)
                    {
                        List<ExternalStaticLinksEx1.iObject> ioNew = _data.GetChildEx(ao, io, tag2, _SERVER);
                        io.n.AddRange(ioNew);
                        nn = ioNew.Count();
                        for (int ii = 0; ii < nn; ii++)
                        {
                            ExternalStaticLinksEx1.iObject ioN = ioNew.ElementAt(ii);
        ...
        private XElement UpdateMissing()
        {
            ...
            try
            {
                if (_lObjects.Where(p => p.id == "" || p.n.Where(q => q.id == "").Count() != 0).Count() != 0)
                {
                    ...
                    foreach (ExternalStaticLinksEx1.iObject io in _lObjects.Where(p => p.id == "" || p.n.Where(q => q.id == "").Count() != 0))
                    {
                        XElement row = null;
                        ...
                        {
                            foreach (ExternalStaticLinksEx1.iObject ioBm in io.n.Where(p => p.id == ""))
                            {
                                XElement bm = null;
