...
namespace DIBIndexingCLR
{
    ...
    public class RegexMatchActions
    {
        ...
        public class MatchAction
        {
            ...
            public RegexAction Actions { get; set; }
        ...
        public class RegexAction
        {
            ...
            {
#pragma warning disable CS0219 // The variable 'line' is assigned but its value is never used
                int line = 0;
#pragma warning restore CS0219 // The variable 'line' is assigned but its value is never used
                try
                {
...
#pragma warning restore CS0168 // The variable 'err' is declared but never used
        ...
        public Markup ExecuteMatchActions(MatchObject m, string language,  XTextRange range)
        {
            ...
            try
            {
                if (m.match.Value.Trim().Length > 0)
                {
                    if (m.processed)
                    {
                    ...
                    else
                    {
                        ...
                        foreach (var group in groups)
                        {
                            ...
                            mark = new Markup(parentId, m.match.Index, m.match.Length, m.name, m.match.Value, m.replace);
                            rg.mark = mark;
                            string parentid = parentId; //m.GetHashCode().ToString(); // AddMarkup(rg, m, ma);
                            ...
                            m.matches= rg.MatchGroups.Select(p => p).StringConcatenate(";");
                            mark.matches = m.matches;
                            rg = null;
                            ma = null;
                        }
    ...
    public class TextProcuctionObject
    {
        ...
        public XElement GetLinks()
        {
            XElement result = new XElement("diblink",
                ...
                (
                    ...
                    //join ilink in Ranges.SelectMany(p=>p.idLinks).Where(p=>p.type==IdLinkType.Link) on mlink.id equals ilink.parent
                ...
                (
                    ...
                    select new
                    {
                        ...
                        marks = mo.matches
                                ...
                                .ToList()
                        //mc.OfType<Match>()
                        //        //.Where(m=> m.Length>0 && !Regex.IsMatch(m.Value.Trim(),@"^[\:\.\,]+$"))
                        //        .Where(m =>
                        //                (xr.Offseth.Between(m.Index, m.Index + m.Length, true))
                        //            || ((xr.Offseth + xr.Length).Between(m.Index, m.Index + m.Length, true))
                        //            || ((xr.Offseth <= m.Index) && ((xr.Offseth + xr.Length) >= (m.Index + m.Length)))
                        //        )
                        //        //.OrderBy(m => m.Index)
                        //        .ToList()
                    }
                            ...
                            if (il.Count() == 0)
                            {
                                range.Add(
                                        new XElement("x-index",
                                            ...
                                            new XAttribute("text", mark.value),
        ...
        private void EvaluateXTexts(IEnumerable<XText> XTexts, List<XText> brtext)
        {
            ...
            string splitt = " | ";
#pragma warning disable CS0219 // The variable 'bSplitt' is assigned but its value is never used
            bool bSplitt = false;
#pragma warning restore CS0219 // The variable 'bSplitt' is assigned but its value is never used
