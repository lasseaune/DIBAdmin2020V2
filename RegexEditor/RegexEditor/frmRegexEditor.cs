...
namespace RegexEditor
{
    public partial class frmRegexEditor : Form
    {
        ...
        private void endreToolStripMenuItem_Click(object sender, EventArgs e)
        {

        }

        private void btnTestRxSources_Click(object sender, EventArgs e)
        {
            XElement iddoc = XElement.Load(@"D:\_RegexEditor\iddoc.xml");

            string rx = iddoc.BuildRegexFromIdDoc();
            XElement result = new XElement("root", rx);
            result.Save(@"D:\_RegexEditor\idpart_new.xml");

            iddoc = XElement.Load(@"D:\_DIBIndexingCLR\iddoc.xml");
            rx = iddoc.BuildRegexFromIdDoc();
            result = new XElement("root", rx);
            result.Save(@"D:\_RegexEditor\idpart_old.xml");
